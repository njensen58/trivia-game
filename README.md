This Project was made as a practice in using React with Redux and Thunk in order to make asynchronous requests to an API.  It also was a challenge in logic to have the possible answers display in a random order, have that order persist over re-rendering, and let the user change question categories.

In addition to using Redux and Thunk, React Router was used for Single Page Application practice.
Media Queries used to make mobile and tablet/desktop adaptiveness.

Trivia questions were provided by the Open Trivia Database API: https://opentdb.com.
Background images are from https://unsplash.com.
Initial React environment created with Create React App, all used dependencies are in package.json.
